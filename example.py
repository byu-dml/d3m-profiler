...
embed.create_save_embeddings(df=pd.read_csv(file_data), model_weights_path='../data_files/distilbert-base-nli', embedding_model='SentenceTransformer', use_col_name_only=True, path_embedding = file_data_embed)

#now start the testing
initialized_models = [RandomForestClassifier(random_state=15),MLPClassifier(random_state=10)]
