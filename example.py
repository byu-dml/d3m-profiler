...
split_type = GroupShuffleSplit(n_splits=9, test_size=0.3, random_state=31)
#create the models
#simon model
...
#random forest model
model_random_forest = MetaDataProfiler(model=model_comp, use_col_name_only=True, embedding_type='SentenceTransformer', EMBEDDING_WEIGHTS_PATH=weights_path, model_name='RandomForest_ColName_GroupShuffle_9', balance_type='SMOTE', balance='True', embed_data_file=meta_embed_path, split_type=split_type, data_path=meta_data_path)
#mlp classifier
model_mlp = MetaDataProfiler(model=model_comp_1, use_col_name_only=True, embedding_type='SentenceTransformer', EMBEDDING_WEIGHTS_PATH=weights_path, model_name='MLP_ColName_GroupShuffle_9', balance_type='SMOTE', balance='True', embed_data_file=meta_embed_path, split_type=split_type, data_path=meta_data_path)

#get both the models to run with the profiler
initialized_models = [model_SIMON, model_random_forest, model_mlp]
