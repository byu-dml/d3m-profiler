...
file_to_save = 'rf_simon_mlp_unbal_4.csv'

np.random.seed(18)
model_meta = RandomForestClassifier(random_state=np.random.randint(25,size=5)[4])
model_both = MLPClassifier(random_state=np.random.randint(25,size=5)[4])
np.random.seed(12)
split_type = GroupShuffleSplit(n_splits=4, test_size=0.3, random_state=np.random.randint(25,size=5)[4])
np.random.seed(15)
model_combined = MetaSimon(data_path=data_path, embed_data_path=embed_path, model_meta=model_meta, model_both=model_both, balance=False, split_type=split_type, seed=np.random.randint(25,size=5)[4])
