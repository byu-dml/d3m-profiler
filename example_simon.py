...
file_to_save = 'simon_tests_4.csv'
np.random.seed(12)
split_type = GroupShuffleSplit(n_splits=4, test_size=0.3, random_state=np.random.randint(25,size=5)[4])
np.random.seed(15)
model_simon_mlp = SimonMLP(split_type=split_type, embed_data_file=embed_path, data_path=data_path, seed=np.random.randint(25,size=5)[4])
np.random.seed(15)
model_simon = BaselineSimon(split_type=split_type, embed_data_file=embed_path, data_path=data_path, seed=np.random.randint(25,size=5)[4])

#get both the models to run with the profiler
initialized_models = [model_simon, model_simon_mlp]
